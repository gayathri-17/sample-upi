...
/**
 * Searchable modal filter picker
 */
export default class ModalFilterPicker extends Component {
  ...
  render() {
    ...
    return (
      ...
      >
        <KeyboardAvoidingView
          behavior="padding"
          style={[styles.overlay, styles.flexContainer]}
  ...
  renderOption = ({ item }) => {
    ...
    if (renderOption) {
      return renderOption(item, value === selectedOption)
    }
    return (
      <TouchableOpacity
        activeOpacity={0.7}
        style={style}
        onPress={() => this.props.onSelect(item, 2)}
      >
